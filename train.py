...
def train(transe, train_dataloader):
...
def define_model(mode):
	if mode.lower() == 'transe':
		...
		model = TransH(
			...
			p_norm = 1, 
			norm_flag = True)
	elif mode.lower() == 'transr':
		# pretrain transe, then train transr
		# model = 
		pass
	return model
